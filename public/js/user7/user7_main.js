...
$(function () {
    ...
    function utilitiesData(response) {
        ...
        $('#utilities_table').grid({
            ...
            columns: [{ field: 'name', title: 'Անուն' }, { field: 'unit', title: 'Միավոր' }, { tmpl: '<input class="num_input amt">', title: 'Քանակ' }, { tmpl: '<input class="num_input price" data-id="{id}" >', title: 'Գումար' }, {
    ...
    function expensesData(data) {
            ...
            dataSource: {
                success: function success() {
                    ...
                    $('.check_active').on('change', function () {
                        ...
                        var elem = $(this).closest('tr').find('select').empty();
                        var good = elem.data('id');
                        var options = '<option value="" selected>Ընտրել Հաճախորդ</option>';
        ...
        function supplierList(value, record, $cell, $displayEl, id, $grid) {
            var sel = '<select class="form-control select" data-id=' + id + '><option value="" selected>\u0538\u0576\u057F\u0580\u0565\u056C \u0574\u0561\u057F\u0561\u056F\u0561\u0580\u0561\u0580</option>';
            $.each(record.supplier, function (i, v) {
                return sel += '<option value="' + v.id + '">' + v.name + '</option>';
            });
            sel += '</select>';
            return sel;
        }
        ...
        $('#expense_btn').off('click').on('click', function () {
            ...
            $('.p_amt').each(function () {
                var price = $(this).closest('tr').find('.p_price');
                ...
                if ($(this).val() != '' && (price.hasClass('disable_amt_inp') ? true : price.val() != '') && select.val() != '') {
                    ...
                    var checkBox = $(this).closest('tr').find('.check_active');
                    data.push((_data$push = {
                        amt: $(this).val(),
                        price: price.val()
                    }, _defineProperty(_data$push, checkBox.prop('checked') ? 'client' : 'supplier', select.val()), _defineProperty(_data$push, 'good_id', good_id), _data$push));
                } else if ((price.hasClass('disable_amt_inp') ? $(this).val() != '' && select.val() == '' : price.val() == '') && $(this).val() != '' && select.val() == '' || (price.hasClass('disable_amt_inp') ? $(this).val() == '' && select.val() == '' : price.val() != '') && $(this).val() == '' && select.val() == '' || (price.hasClass('disable_amt_inp') ? $(this).val() == '' && select.val() != '' : price.val() == '') && $(this).val() == '' && select.val() != '' || (price.hasClass('disable_amt_inp') ? $(this).val() != '' && select.val() == '' : price.val() != '') && $(this).val() != '' && select.val() == '' || (price.hasClass('disable_amt_inp') ? $(this).val() == '' && select.val() != '' : price.val() != '') && $(this).val() == '' && select.val() != '' || (price.hasClass('disable_amt_inp') ? $(this).val() != '' && select.val() != '' : price.val() == '') && $(this).val() != '' && select.val() != ''
            ...
            if (data.length != 0) {
                $.post('user7/accept_expenses', { data: data }, function (response) {
                    if (response.status == 'success') {
                        FadeInModal({
                            el: modal,
                            title: '',
        ...
        var ret_grid = $('#returned_good_table').grid({
            ...
            responsive: true,
            notFoundText: 'Արդյունք չի գտնվել',
            columns: [{ field: 'supplier_name', title: 'Մատակարար' }, { tmpl: '<button type="button" class="btn form-control btn-primary accessReturned" data-id="{supplier_id}">Հաստատել</button>', cssClass: 'fa_button', width: '150px' }]
        });
        ...
        function onSuccess(response) {
            ...
            $('.accessReturned').on('click', function () {
                ...
                $.post('user7/access_returned_good', { id: id }, function (response) {
        ...
        ret_grid.on('detailExpand', function (e, $detailWrapper, id) {
            var detail = $detailWrapper.find('table').grid({
                ...
                fontSize: 15,
                fixedHeader: true,
                columns: [{ field: 'good_name', title: 'Ամսաթիվ' }, { field: 'good_unit', title: 'Միավոր' }, { field: 'amt', title: 'Քանակ', editor: true }, { width: 200, align: 'center', renderer: editManager, cssClass: 'fa_button' }]
            });

            detail.on('rowDataChanged', function (e, id, record) {
                $.post('user7/update_returned_good', { record: record });
            });
        ...
        var paid_grid = $('#paid_good_table').grid({
            dataSource: { url: 'user7/pay_good_data', success: function success(response) {
                    log(response);
                    var record = [];
                    ...
                    $('.pay_good_button').off('click').on('click', function () {
                        ...
                        $('.confirm_pay_good').on('click', function () {
                            $.post('user7/access_pay_good', { id: id }, function (response) {
                                if (response.status == 'success') {
                                    paid_grid.reload();
                                }
                            });
            ...
            notFoundText: 'Արդյունք չի գտնվել',
            detailTemplate: '<div><table  style="background:#fcf8e3"></div>',
            columns: [{ field: 'supplier_name', title: 'Մատակարար' }, { field: 'sum', title: 'Գործարքի գումարը' }, { tmpl: '<button type="button" class="btn btn-primary pay_good_button" data-id="{supplier_id}">Վճարել</button>', cssClass: 'fa_button', width: 200 }]
        });
        paid_grid.on('detailExpand', function (e, $detailWrapper, id) {
            var detail = $detailWrapper.find('table').grid({
                ...
                notFoundText: 'Արդյունք չի գտնվել',
                fixedHeader: true,
                columns: [{ field: 'good_name', title: 'Ապրանք', cssClass: 'bold' }, { field: 'good_unit', title: 'Միավոր' }, { field: 'amt', title: 'Քանակ' }, { field: 'good_price', title: 'Գին' }, { field: 'cost', title: 'Գումար' }, { field: 'date', title: 'Ընդունման ամսաթիվ' }]
            });
        });
    }
    ...
    function paidExpensesHistory() {
        ...
        var expenses_grid = $('#grid_history_expenses').grid({
            dataSource: {
                url: 'user7/paid_goods_history', type: 'post', success: function success(response) {
                    var records = [];
                    for (var x in response) {
                        var sum = 0;
                        for (var j in response[x]) {
            ...
            columns: [{ field: 'supplier_name', title: 'Մատակարար' }, { field: 'sum', title: 'Գործարքի գումարը ' }]
        });
        expenses_grid.on('detailExpand', function (e, $detailWrapper, id) {
            var detail = $detailWrapper.find('table').grid({
                ...
                columns: [{ field: 'good_name', title: 'Ապրանք', cssClass: 'bold' }, { field: 'good_unit', title: 'Միավոր' }, { field: 'amt', title: 'Քանակ' }, { field: 'good_price', title: 'Գին' }, { field: 'cost', title: 'Գումար' },
        ...
        var utilities_grid = $('#grid_history_utilities').grid({
            dataSource: {
                url: 'user7/paid_utilities_history', type: 'post', success: function success(response) {
                    var records = [];
                    for (var x in response) {
                        var sum = 0;
                        for (var j in response[x]) {
            ...
            detailTemplate: '<div><table  style="background:#fcf8e3"></div>',
            params: { name: input, from: from, to: to },
            columns: [{ field: 'month_name', title: 'Ամիս' }, { field: 'sum', title: 'Ամսվա գործարքի ընհանուր գումարը ' }]
        });
        utilities_grid.on('detailExpand', function (e, $detailWrapper, id) {
            var detail = $detailWrapper.find('table').grid({
                ...
                columns: [{ field: 'expense_name', title: 'Ծախսի տեսակ', cssClass: 'bold' }, { field: 'amt', title: 'Քանակ' }, { field: 'balance', title: 'Գումարը' }, { field: 'date', title: 'Վճարման ամսաթիվ' }]
            });
        });
    ...
    function getOrdersInfo() {
        ...
        grid.on('detailExpand', function (e, $detailWrapper, id) {
            var detail = $detailWrapper.find('table').grid({
                ...
                fontSize: 15,
                fixedHeader: true,
                columns: [{ field: 'date', title: 'Ամսաթիվ <i class="fas fa-sort"></i>', sortable: true }, {
                    field: groupBy('product_name', 'client_name'),
                    title: groupBy('Ապրանք ', 'Հաճախորդ ') + '<i class="fas fa-sort"></i>',
                    sortable: true
                }, { field: 'product_amt', title: 'Քանակ <i class="fas fa-sort"></i>', sortable: true }, { field: 'order_price', title: 'Գումար <i class="fas fa-sort"></i>', sortable: true }]
            });
    ...
    function getClientsInfo() {
        ...
        var grid = $('#client_table').grid({
            dataSource: {
                url: 'user7/client_data', type: 'post', success: function success(response) {
            ...
            columns: [{ field: 'name', title: 'Անուն <i class="fas fa-sort"></i>', editor: true, sortable: true }, { field: 'address', title: 'Հասցե <i class="fas fa-sort"></i>', editor: true, sortable: true }, { field: 'phone', title: 'Հեռախոս', editor: true }, {
    ...
    function getExpensesInfo() {
        ...
        var grid = $('#expenses_table').grid({
            ...
            autoLoad: false,
            inlineEditing: { mode: 'command', managementColumn: false },
            columns: [{ field: 'name', title: 'Անուն <i class="fas fa-sort"></i>', editor: true, sortable: true }, { field: 'unit', title: 'Միավոր<i class="fas fa-sort"></i>', editor: true, sortable: true }, { width: 200, align: 'center', renderer: editManager, cssClass: 'fa_button' }],
            pager: { limit: 10, sizes: [5, 10, 20, 50] }
        });
    ...
    function getGoodsInfo(response) {
        ...
        var defSuppliers = response.suppliers;
        ...
        var grid = $('#good_table').grid({
            dataSource: { url: 'user7/good_data', type: 'post', success: function success(response) {
                    for (var x in response) {
                    ...
                    multiSelect.multiselect('disable');
                    $.each(response.records, function (k, v) {
                        var elem = $('.basic[data-id=' + v.id + ']');
                        var selected_id = JSON.parse(elem.attr('data-selected-id'));
                        elem.multiselect('select', selected_id);
                    });
                } },
            primaryKey: 'id',
            ...
            notFoundText: 'Արդյունք չի գտնվել',
            inlineEditing: { mode: 'command', managementColumn: false },
            columns: [{ field: 'name', title: 'Անուն <i class="fas fa-sort"></i>', editor: true, sortable: true }, { field: 'unit', title: 'Միավոր <i class="fas fa-sort"></i>', editor: true, sortable: true }, { renderer: multiSelects, title: 'Մատակարար' }, {
        ...
        function multiSelects(value, record, $cell, $displayEl, id, $grid) {
            ...
            var ids = JSON.stringify(record.supplier.map(function (item) {
                return item.id;
            }));

            var multiSelectValue = document.createElement('select');
            ...
            for (var i = 0; i < defSuppliers.length; i++) {
                var multiOptionValue = document.createElement('option');
    ...
    function getProductInfo() {
        ...
        $('#btnAddRow').off().on('click', function () {
            ...
            $('.add_new_product_table').append('<tr>\n' + '<td><input class="gj-textbox-md modal_input new_product_name"></td>' + '<td><input class="gj-textbox-md modal_input new_product_height"></td>' + '<td><input class="gj-textbox-md modal_input new_local_price"></td>' + '<td><input class="gj-textbox-md modal_input new_export_price"></td>' + '<td><button class="remove_row"><i class="fas fa-minus-circle"></i></button></td>' + '</tr>');
        ...
        var grid = $('#product_table').grid({
            ...
            notFoundText: 'Արդյունք չի գտնվել',
            inlineEditing: { mode: 'command', managementColumn: false },
            columns: [{ field: 'name', title: 'Անուն <i class="fas fa-sort"></i>', editor: true, sortable: true }, { field: 'height', title: 'Բոյ <i class="fas fa-sort"></i>', editor: true, sortable: true }, {
                field: 'local_price',
                title: 'Արտահանման գին <i class="fas fa-sort"></i>',
                editor: true,
                sortable: true
            }, { field: 'export_price', title: 'Տեղական գին <i class="fas fa-sort"></i>', editor: true, sortable: true }, { width: 200, align: 'center', renderer: editManager, cssClass: 'fa_button' }],
            pager: { limit: 10 }
        });
    ...
    function getAcceptInfo() {
        ...
        table.grid({
            ...
            height: 630,
            fontSize: 15,
            columns: [{ field: 'name', title: 'Անուն ' }, { field: 'address', title: 'Հասցե' }, { field: 'phone', title: 'Հեռախոս' }, { title: 'Գումար', tmpl: '<input class="num_input paid" data-id="{id}">' }]
        });
        $('#btn_accept').off('click').on('click', function () {
            var input = table.find('input.paid');
            var modal = $('#accept_dialog');
            var data = [];
            $.each(input, function () {
